...
class Tip(Base):
    ...
    owner_type = Column(String(20), default='Tipster') # 'Tipster', 'User', 'System'
...
def analyze_system_edge(tipster_id, odds_tipster, odds_opp, current_line, market_type, sys_bankroll, session, kelly_fraction=0.25):
    X, y = get_tipster_features(tipster_id, session)
    
    if X is None or len(X) < 10:
        p_win_tipster = 0.5
        ai_status = "數據不足，採用基礎賠率機率"
    else:
    ...
    # 決定系統方向
    if ev_opp > ev_tipster and ev_opp > 0:
        sys_choice = 'Opposite'
        target_p = p_win_opp
...
def get_avail_bankroll(account_type, tipster_id, session):
    ledgers = session.query(BankrollLedger).filter_by(account_type=account_type)
    if tipster_id: ledgers = ledgers.filter_by(tipster_id=tipster_id)
    ledgers = ledgers.all()
    
    tips = session.query(Tip).filter(Tip.status != 'Open', Tip.is_deleted == False, Tip.owner_type == account_type)
    if tipster_id: tips = tips.filter_by(tipster_id=tipster_id)
    tips = tips.all()
...
def preview_db_dialog():
    ...
    main_tabs = st.tabs(["📝 投注記錄", "💰 資金流水", "⏪ 撤銷與回滾"])
    
    # --- 投注記錄分類 ---
    with main_tabs[0]:
        sub_tabs = st.tabs(["👑 系統 (System)", "👤 用家 (User)"] + t_names)
        
        entities = [('System', None)] + [('User', None)] + [('Tipster', t.id) for t in tipsters]
        
        for i, (owner_type, t_id) in enumerate(entities):
            with sub_tabs[i]:
                ...
                for t in tips:
                    ...
                    data.append({
                        'ID': t.id, '賽事': t.tournament, '對陣': f"{t.home_team} vs {t.away_team}",
                        '盤口': f"{t.market_type} ({t.line})", '選項': t.selection, '賠率': t.odds,
                        '投注額': t.stake, '結果': res, '狀態': t.status, '盈虧': t.profit
                    })
                
                df = pd.DataFrame(data)
                df['賠率'] = df['賠率'].astype(float)
                df['投注額'] = df['投注額'].astype(float)
                df['盈虧'] = df['盈虧'].astype(float)
                st.dataframe(df.style.map(style_financials, subset=['盈虧']).format({'賠率': "{:.2f}", '投注額': "{:.2f}", '盈虧': "{:.2f}"}), use_container_width=True)
    ...
    # --- 資金流水分類 ---
    with main_tabs[1]:
        sub_tabs2 = st.tabs(["👑 系統 (System)", "👤 用家 (User)"] + t_names)
        for i, (owner_type, t_id) in enumerate(entities):
            with sub_tabs2[i]:
...
def main():
    ...
    with tabs[-1]:
        ...
        st.button("🗄️ 開啟數據庫管理 (預覽 / 撤銷)", on_click=preview_db_dialog)
        st.divider()
        
        c_m1, c_m2 = st.columns(2)
        with c_m1:
            st.write("🙈 **隱藏分享者 Tab** (不會刪除數據)")
        ...
        with c_m2:
            ...
            hide_team_opts = st.multiselect("隱藏特定的隊伍名稱", clean_teams)
            hide_tour_opts = st.multiselect("隱藏特定的賽事名稱", clean_tours)
            
            if st.button("確認加入選單黑名單"):
    ...
    for i, tipster in enumerate(tipsters):
        with tabs[i]:
            ...
            with colL:
                ...
                with c_o2:
                    st.number_input(f"賠率 ({get_opposite_selection(selection)})", min_value=1.01, step=0.01, key="odds2")
                with c_o3:
            ...
            with colR:
                ...
                if st.button("🧠 執行 AI 深度分析與系統策略", key=f"run_ai_{tipster.id}", use_container_width=True):
                    res = analyze_system_edge(tipster.id, st.session_state.odds1, st.session_state.odds2, line, market, sys_bankroll, db_session)
            ...
            if st.button("🚀 確認提交", key=f"submit_{tipster.id}", type="primary", use_container_width=True):
        ...
        if not open_tips:
            ...
            for m_key, m_tips in matches.items():
                with st.expander(f"📍 {m_key} (共包含 {len(m_tips)} 張注單)"):
                    st.write("**包含的注單細節：**")
                    for t in m_tips:
                        st.caption(f"- [{t.owner_type}] 盤口: {t.market_type}({t.line}) | 選項: {t.selection} | 賠率: {t.odds} | 投注: ${t.stake}")
